setwd("~/OneDrive/MORU/Projects/PSA of Savannakhet model/SA_v9/USA_v9_1/submitted_v/") #mac
...
barplot(incidenceRange[,1], names.arg = incidenceRange[,3], las=1, horiz=T, xlim=c(-.4,.1), beside = T, axes=F, col='light blue', main="Sensitivity on incidence, Savannakhet model \n % reduction in incidence", xlab='% reduction in incidence')
barplot(incidenceRange[,2], horiz=T, axes=F, beside=T, add=T, col=adjustcolor('gold',alpha.f = .7))
axis(1, at=seq(-.4,.1,by=.01), labels = 100*seq(-.4,.1,by=.01)+round(default.result[,1]*100,0), ylab='% reduction in incidence')
legend(x=-.3,y=8,legend=c('lower value of parameter','higher value of parameter'), fill=c('light blue','gold'), cex=1.5)
dev.off()
...
par(mar=c(5, 7, 4, 2), cex=1.5)
barplot(prevalenceRange[,1], names.arg = prevalenceRange[,3], axes=F, las=1, horiz=T, xlim=c(-.15,.04), beside = T, col='light blue', main="Sensitivity analysis, Savannakhet model \n % reduction in prevalence", xlab='% reduction in prevalence')
barplot(prevalenceRange[,2], horiz=T, axes=F, beside=T, add=T, col=adjustcolor('gold',alpha.f = .7))
axis(1, at=seq(-.15,.04,by=.01), labels = 100*seq(-.15,.04,by=.01)+round(default.result[,2]*100,0), ylab='% reduction in prevalence')
legend(x=-.135,y=8,legend=c('lower value of parameter','higher value of parameter'), fill=c('light blue','gold'), cex=1.5)
dev.off()