...
barplot(incidenceRange[,1], names.arg = incidenceRange[,3], las=1, horiz=T, xlim=c(-.65,0.1), beside = T, axes=F, col='light blue', main=paste("Sensitivity on incidence, Savannakhet model \n vaccinate all:", !v_same), xlab='% reduction in incidence')
barplot(incidenceRange[,2], horiz=T, axes=F, beside=T, add=T, col=adjustcolor( "gold", alpha.f = 0.7)) #col='gold', alpha=.8)
axis(1, at=seq(-.65,0.1,by=.1), labels = seq(-.65,0.1,by=.1)+round(default.result[,1],2), ylab='% reduction in incidence')
