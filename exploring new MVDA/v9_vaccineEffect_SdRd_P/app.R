...
runGMS<-function(initprev, scenario, param) 
{
  ...
  SdRd <- c(11,12,19,20,27,28,35,36,43,44)
  # population
  ...
  GMSout[[j]]<-matrix(NA,nrow=length(times),ncol=5)
  GMSout[[j]][,1]<-times
  GMSout[[j]][,2]<-clinmonth_det
  GMSout[[j]][,3]<-clinmonth_tot
  GMSout[[j]][,4]<-prevalence
  GMSout[[j]][,5]<-rowSums(out[,SdRd])/pop
  }
  return(GMSout)
}
