...
resultTable <- data.frame(cbind(resultList[[1]],resultList[[2]],resultList[[3]],resultList[[4]],resultList[[5]],resultList[[6]]))
xlabels <- c("Base","EDAT","ITN","MDA","VAC","MSAT")
...
par(cex=1.5)
boxplot(resultTable[,seq(1,12,by=2)], main='detectedIncidence/1000/month')
dev.off()

png(file=paste('result/PrevalenceByIntervention_',gsub(':','_',Sys.time()),'.png',sep = ''), width = 1280, height = 800)
par(cex=1.5)
boxplot(resultTable[,seq(2,12,by=2)], main='TruePrevalence')
dev.off()
...
incId <- cbind(1:nrow(resultTable),resultTable[,seq(1,12,by=2)])
names(incId)[1] <- 'id'
...
g <- g+geom_point(data = incIdMolten[seq(102, by=102, length=6),], aes(shape=factor(id)))
g
dev.off()

#reshaping data for line plots with facets
resultTable2 <- resultTable #[-length(resultTable),]
incId <- cbind(1:nrow(resultTable2),rep(1:6, each=1700),resultTable2[,seq(1,12,by=2)])
...
prevId <- cbind(1:nrow(resultTable2),resultTable2[,seq(2,12,by=2)])
names(prevId)[1] <- 'id'
...
g <- g+geom_point(data = prevIdMolten[seq(102, by=102, length=6),], aes(shape=factor(id)))
g
dev.off()

#reshaping data for line plots with facets
resultTable2 <- resultTable #[-length(resultTable),]
prevId <- cbind(1:nrow(resultTable2),rep(1:6, each=1700),resultTable2[,seq(1,12,by=2)])
...
# 
# #PLOT
# png(file=paste('result/IncidenceByVACCINATION_',gsub(':','_',Sys.time()),'.png',sep = ''), width = 1280, height = 800)
# par(cex=1.5)
# boxplot(resultTable[,seq(1,4,by=2)], main='detectedIncidence/1000/month')
# dev.off()
# 
# png(file=paste('result/PrevalenceByVACCINATION_',gsub(':','_',Sys.time()),'.png',sep = ''), width = 1280, height = 800)
# par(cex=1.5)
