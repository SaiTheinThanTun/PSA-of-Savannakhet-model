setwd("~/OneDrive/MORU/Projects/PSA of Savannakhet model/truePSA/output_Inc_Prev/") #mac
#setwd("D:/OneDrive/MORU/Projects/PSA of Savannakhet model/truePSA/output_Inc_Prev/") #windows
library(deSolve)
library(shiny)
library(TSA)
library(Rcpp)
...
simValueTable <- read.csv('result/plausibleParamFinal151.csv')
...
par(cex=1.5)
boxplot(resultTable[,seq(1,14,by=2)], main='TrueIncidence/1000/month')
dev.off()
...
g+geom_point()+geom_line(colour=rainbow(1057))+ggtitle('Incidence/1000/Month')
dev.off()

#reshaping data for line plots with facets
resultTable2 <- resultTable[-length(resultTable),]
incId <- cbind(1:nrow(resultTable2),rep(1:6, each=25),resultTable2[,seq(1,14,by=2)])
...
prevId <- cbind(1:nrow(resultTable),resultTable[,seq(2,14,by=2)])
names(prevId)[1] <- 'id'
...
g+geom_point()+geom_line(colour=rainbow(1057))+ggtitle('Prevalence')
dev.off()

#reshaping data for line plots with facets
resultTable2 <- resultTable[-length(resultTable),]
prevId <- cbind(1:nrow(resultTable2),rep(1:6, each=25),resultTable2[,seq(1,14,by=2)])
